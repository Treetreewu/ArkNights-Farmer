...
def farm():
    ...
    for count in range(limit):
        ...
        touch(Template(resource_path("image/tpl1561777675953.png"), record_pos=(0.401, 0.202), resolution=(2160, 1080)))
...
def recruit():
    pos = exists(Template("image/tpl1561945272209.png", record_pos=(0.295, 0.109), resolution=(2160, 1080)))
    if not pos:
        print("放在主页哦~亲。")
        return
    touch(pos)
    while True:
        pos = exists(Template("image/tpl1561945278845.png", record_pos=(-0.216, 0.016), resolution=(2160, 1080)))
        global SKIP_POS
        if pos:
            touch(pos)
            sleep(1)
            if not SKIP_POS:
                SKIP_POS = touch(Template("image/tpl1561945291549.png", record_pos=(0.459, -0.22), resolution=(2160, 1080)))
            else:
                touch(SKIP_POS)
            sleep(2)
            touch(SKIP_POS)
...
def done_task():
    def done_tab():
        last_pos = None
        while True:
            pos = exists(Template("image/tpl1562122652880.png", record_pos=(0.295, -0.151), resolution=(2160, 1080)))
            if pos:
                last_pos = pos
                touch(pos)
                sleep(1)
...
def gay_friends():
    touch(Template("image/tpl1562121233580.png", record_pos=(-0.246, 0.15), resolution=(2160, 1080)))
    touch(Template("image/tpl1562121247202.png", record_pos=(-0.415, -0.096), resolution=(2160, 1080)))
    touch(Template("image/tpl1562121289070.png", record_pos=(0.184, -0.133), resolution=(2160, 1080)))
    sleep(5)
    while True:
        pos = exists(Template("image/tpl1562117180144.png", record_pos=(0.433, 0.183), resolution=(2160, 1080)))
        if pos:
            touch(pos)
            sleep(3)
        else:
...
def press_back(times):
    ...
    for i in range(times):
        if BACK_POS:
            ...
            BACK_POS = touch(Template("image/tpl1562117190883.png", record_pos=(-0.441, -0.223), resolution=(2160, 1080)))
        sleep(0.5)
...
def go_home(in_gay=False):
    ...
    if HOME_POS and False:
        ...
        HOME_POS = [
            touch(Template("image/tpl1562119328762.png", record_pos=(-0.312, -0.223), resolution=(2160, 1080))),
            touch(Template("image/tpl1562119339267.png", record_pos=(-0.38, -0.088), resolution=(2160, 1080)))
        ]
    sleep(0.5)
    if in_gay:
        pos = exists(Template("image/tpl1562120546615.png", record_pos=(0.176, 0.1), resolution=(2160, 1080)))
        if pos:
            touch(pos)
        sleep(5)
...
def main():
    task = input("""请选择功能:
    （默认234, 现在可以自由组合任务 例如 123，但有1时，1必须在第一位）
    1: 刷图
    2: 收招募
    3: 收任务
    4: 好友基♂建
...
if __name__ == '__main__':
    ...
    try:
        ...
        ip = ip.replace("：", ":")
        try:
            os.system("airtest\\core\\android\\static\\adb\\windows\\adb.exe connect {}".format(ip))
            connect_device("android:///{}?cap_method=javacap".format(ip))
        except:
