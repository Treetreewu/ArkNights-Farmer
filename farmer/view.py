...
from farmer.task import TasksRunner, TASKS
from farmer.utils import StringUtils, resource_path
...
class FarmTaskWindow(BaseDialogWindow):
    def __init__(self, parent, **kwargs):
        ...
        self.auto_drink = ttk.Checkbutton(self.tk, text=s.auto_drink,
                                          variable=self.auto_drink_var)
        self.auto_eat_var = tk.IntVar(self.tk, value=0)
        self.auto_eat = ttk.Checkbutton(self.tk, text=s.auto_eat,
                                        variable=self.auto_eat_var)
...
class GayWindow(BaseDialogWindow):
    def __init__(self, parent, **kwargs):
        ...
        self.auto_drone = ttk.Checkbutton(self.tk, text=s.use_drone,
                                          variable=self.auto_drone_var,
                                          command=self.on_auto_drone_change)
        ...
        self.drone = ttk.OptionMenu(
            self.tk, self.drone_var, s.drone_bill,
            *[s.drone_bill, s.drone_produce], command=self.update_drone_detail
        )
        self.drone.configure(state="disabled")
        self.drone_detail_var = tk.StringVar(self.drone_frame.tk)
        self.drone_detail = ttk.OptionMenu(self.drone_frame.tk,
                                           self.drone_detail_var)
...
class NetADBWindow(BaseDialogWindow):
    def __init__(self, parent, **kwargs):
        ...
        self.entry = ttk.Entry(self.tk, validate="key")
...
class RecruitWindow(BaseDialogWindow):
    def __init__(self, parent, **kwargs):
        super().__init__(parent, **kwargs)
        self.tk.title(s.recruit_title)
        self.auto_start_recruit_var = tk.IntVar(self.tk, value=1)
        self.auto_start_recruit = ttk.Checkbutton(
            self.tk, text=s.auto_start_recruit,
            variable=self.auto_start_recruit_var
        )
        self.over_5_var = tk.IntVar(self.tk, value=0)
        self.over_5 = ttk.Checkbutton(self.tk, text=s.recruit_over_5,
                                      variable=self.over_5_var)
        self.button = ttk.Button(self.tk, text=s.ok, command=self.on_submit)

        # self.map_.pack(padx=10, pady=(15, 0))
        self.auto_start_recruit.pack(padx=10, pady=(10, 0))
        self.over_5.pack(padx=10, pady=(10, 0))
        self.button.pack(side=tk.LEFT, padx=(10, 20), pady=10, ipady=5)

    def on_submit(self):
        try:
            config = {
                "auto_recruit": bool(self.over_5_var.get()),
                "choosable_over_5": bool(self.over_5_var.get()),
            }
        except ValueError:
            messagebox.showerror(s.error, s.input_wrong)
            return
        self.parent.add_task("recruit", config)
        self.tk.destroy()
...
class DonationWindow(BaseDialogWindow):
    def __init__(self, parent, **kwargs):
        ...
        wechat_qr_png = ImageTk.PhotoImage(
            Image.open(resource_path("image/gui/wechat_qr.png")))
        alipay_qr_png = ImageTk.PhotoImage(
            Image.open(resource_path("image/gui/alipay_qr.png")))
...
class BottomBarFrame(BaseFrame):
    def __init__(self, parent):
        ...
        self.device_menu = ttk.OptionMenu(self.tk, self.device_menu_var,
                                          direction='above',
    ...
    def run(self, _=None):
        ...
        try:
            serial_number = re.search(
                "\\((.*?)\\)$",
                self.parent.bottom_bar.device_menu_var.get()
            ).group(1)
        except:
...
class TaskFrame(BaseFrame):
    def __init__(self, parent):
        ...
        self.task_list.item2str = lambda task: TASKS[task["type"]][
            "str"].format(**(task.get("config")) or {})
        self.update()
        # TODO edit task
...
class TaskButtonsFrame(BaseFrame):
    def __init__(self, parent):
        ...
        for t in TASKS:
            button = ttk.Button(text=TASKS[t]["add_str"],
                                command=self.on_click(t))
            button.pack(side=tk.LEFT, in_=self.tk, expand=True, fill=tk.BOTH,
                        ipady=6)
...
class MenuBar:
    ...
    def __init__(self, parent):
        ...
        self.about_menu.add_command(label=s.donate,
                                    command=lambda: DonationWindow(
                                        self.parent))
        # TODO: auto update software.
        self.about_menu.add_command(label=s.check_update,
                                    command=lambda: open_with_browser(
                                        s.github_release_url))
        self.about_menu.add_command(label=s.github,
                                    command=lambda: open_with_browser(
                                        s.github_url))
